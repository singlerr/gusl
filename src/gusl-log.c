...
#define DEFAULT_DOMAIN_PREFIX "gusl"
                           ...
                           int            verbosity_level)
{
  ...
  if (log_level & GUSL_LOG_LEVEL_TRACE)
            ...
            GLogLevelFlags  log_level)
{
  ...
  log_level = log_level & ~GUSL_LOG_DETAILED;
  ...
  if (use_color)
    {
      switch ((int)log_level)        /* Same colors as used in GLib */
        ...
        case GUSL_LOG_LEVEL_TRACE:  return "   \033[1;36mTRACE\033[0m";
  ...
  else
    {
      switch ((int)log_level)
        ...
        case GUSL_LOG_LEVEL_TRACE:  return "   TRACE";
        default:                   return " UNKNOWN";
        }
    }
}
...
gusl_log_write (GLogLevelFlags   log_level,
               const char      *log_domain,
               ...
               gpointer         user_data)
{
  ...
  if (log_level & GUSL_LOG_DETAILED)
    {
...
static GLogWriterOutput
gusl_log_handler (GLogLevelFlags   log_level,
                 const GLogField *fields,
                 gsize            n_fields,
                 gpointer         user_data)
{
  ...
  return gusl_log_write (log_level, log_domain, log_message,
                        fields, n_fields, user_data);
}

static void
gusl_log_finalize (void)
{
  ...
  if (g_once_init_enter (&initialized))
    {
      ...
      g_log_set_writer_func (gusl_log_handler, NULL, NULL);
      g_once_init_leave (&initialized, 1);
      atexit (gusl_log_finalize);
    }
}

void
gusl_log_increase_verbosity (void)
{
  verbosity++;
  enable_backtrace ();
}
...
int
gusl_log_get_verbosity (void)
{
  return verbosity;
}

void
gusl_log_to_file (const char *file_path,
                 gboolean    append)
{
...
const char *
gusl_log_bool_str (gboolean value,
         ...
         ...)
{
  ...
  gusl_log_anonymize_value (str, value);
...
void
gusl_log_anonymize_value (GString    *str,
                         const char *value)
{
